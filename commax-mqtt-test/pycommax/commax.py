...
def do_work(config, device_list):
    ...
    async def recv_from_HA(topics, value):
        ...
        device_raw = topics[1]
        device = re.sub(r'\d+', '', device_raw)
        idx = int(''.join(re.findall(r'\d+', topics[1])))
        ...
        if device == 'Thermo':
            ...
            if topics[2] == 'power':
                # --- [수정된 난방기 ON/OFF 로직] ---
                if value.upper() == 'ON':
                    # ON 명령은 목표 온도를 포함해야 하므로 'CHANGE' 로직 사용
        ...
        if data:
            ...
            if device_name == 'Thermo':
                ...
                if data in stateON_list:
                    ...
                    log(f"[DEBUG] 수신된 패킷: {data} → 속도: {['low', 'medium', 'high'][speed]}")
                ...
                for idx in range(len(DEVICE_LISTS[device]['list'])):
                    ...
                    if DEVICE_LISTS[device]["type"] == "climate":
                        ...
                        payload = {
                            ...
                            "percentage_state_topic": "commax/Fan1/percentage/state", # percentage_state_topic 수정
                            "speed_range_min": 1,
                            "speed_range_max": 3,
                            "payload_off": "OFF",
                            "payload_on": "ON"
    ...
    def on_message(client, userdata, msg):
        ...
        try:
            ...
            log(f"[ERROR] on_message handling failed: {e}")

    async def send_to_elfin():
        while True:
            try:
                # 1. EW11 Keep-Alive / Restart Logic (10초 신호 미수신 시)
                ...
                # 2. Command Queue 처리 로직
                if QUEUE:
                    que = QUEUE[0]
                    sendcmd = que['sendcmd']
                    
                    # 시리얼 통신 대기 시간을 고려 (0.5초)
